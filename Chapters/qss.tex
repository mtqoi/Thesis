...
\paragraph{Function $\mathcal{F}$}

\par

\noindent We have no requirement that $\mathcal{F}$ should be injective, which implies, for example, that the entropy $\text{S}\left(\rho_{A_B, A_C}\right) \ne \text{S}\left(\rho_{X_A}\right)$, and so we must carefully consider the action of $\mathcal{F}$ early on in our analysis. To be concrete, in what follows we assume that $F$ is linear
\begin{equation}\label{eqn:qss_F_linear}
\mathcal{F}\left(x, y\right) := g x + h y \qq{with} g, h \in \mathbb{R}\setminus \left\{0\right\}
\end{equation} 
which will enable us to make some predictions about the performance of the protocol. Although we make no claims about the optimality of this choice of $\mathcal{F}$, we are free to optimize over $g, h$, and we will make it clear when we have done so. In Sec.~\ref{appendix:qss_moreF} we consider some alternative forms for $\mathcal{F}$.





\paragraph{Expanding classical probabilities}
\par
\noindent Applying Bayes' formula Eq.~\ref{eqn:intro_bayes} to the second of these, we see that
\begin{align}
...
\noindent We can access $\text{P}\left(X_A=a \given X_B=b, X_C=c\right)$ by modelling the effects of the channel on quantum states distributed by Bob and Charlie, which we now do. We take
