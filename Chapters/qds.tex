...
\noindent where in the first two inequalities we have applied Eq.~\ref{eqn:qds_hoeffding1} and in the second two inequalities we have applied Eq.~\ref{eqn:hoeffding2}.

Now, defining $p := \text{max}\left\{p_B, p_c\right\}$ we have

\begin{equation}
\text{min}\left\{\text{P}\left(E_A\right), \text{P}\left(E_B\right)\right\} \le \text{exp}\left(- \left[ p - s_B\right]^2 L\right)
\end{equation}
and
\begin{equation}
\text{P}\left(E_C\right) + \text{P}\left(E_D\right) \le 2 \text{exp}\left(-\left[s_C - p\right]^2 L\right).
\end{equation}

So finally we arrive at 

\begin{equation}
\varepsilon_{\text{repudiation}} \le \text{min}\left\{ 2 \text{exp}\left( - \left[p - s_B\right]^2 L \right), \text{exp}\left( - \left[s_C - p\right]^2 L \right) \right\}
\end{equation}

where we have doubled the first entry for convenience. 
