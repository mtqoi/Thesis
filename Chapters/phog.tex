...
Since it is NCL which drives $\rho$ most effectively towards highly sub-Poissonian states we will seek to maximise $\gamma_3$. We fix $G = \sqrt{g_a^2 + g_b^2}$ and define $g_a = x G, g_b = \sqrt{1-x^2}G$ for $0 < x < 1$. Substituting these couplings into $\gamma_3$ we see that
\begin{equation}
...
We demonstrate that the two-mode model gives rise to the same behaviour as the single-mode model in Fig.~\ref{fig:phog_two_mode_single_mode_comparison}. \MT{Additional comments.}
