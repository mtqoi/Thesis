...
Let us conclude by displaying the two signature behaviours of NCL under the single-mode model, using parameters which in subsequent sections we will show to be realistic choices. For reasons which will become apparent, we take \MT{$\gamma$ parameters} and solve
...
\MT{Basically just reproduce Fig.3 from phog paper.}
\MT{TODO: check whether I actually need the Hamiltonian or not}
...
\MT{TODO: show this calculation here.}


And so we have reduced our three-mode model to a two-mode system. Introducing the following collective symmetric and antisymmetric modes
...
\end{align}
with $G = \sqrt{ g_a^2 + g_b^2 }$ required to ensure that $\hat{s}_-, \hat{s}_+$ obey the same bosonic commutation relations as $\hat{a}, \hat{b}$. Rewriting Eq.~\MT{X} in terms of these new modes we arrive at our two-mode model
