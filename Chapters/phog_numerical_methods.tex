...
The above procedure is repeated many times, and observables should be averaged\footnote{Simply using \code{numpy.mean} at each timestep} over many trajectories. It can be shown that this procedure implements the corresponding Lindblad master equation \cite{Plenio1998, Ulf2010} by noticing that the evolution from $\dyad{\psi}\left(t\right)$ to $\dyad{\psi}\left(t + \Delta t\right)$ is effectively a sum over whether a jump occurred or not:
\begin{align}
