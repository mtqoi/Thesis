...
On a standard home-use laptop\footnote{Intel(R) Core(TM) i$5-3230$M CPU $@ 2.60$~GHz; $8.00$~GB RAM} a single-mode system with $\dims = 35$, $\rho\left(0\right)$ a coherent state with $\alpha = 3.0$, decay rate $\gamma = 1.0$, collapse operator $\hat{a}$ and free Hamiltonian $\hat{H} = \omega \hat{a}^\dagger \hat{a}$ with $\omega = 1.0$ can be solved in $41.7$~ms $\pm 11.7$~ms\footnote{Timed via iPython \code{\%timeit} magic command.}. Setting $\dims = 50$ takes $92.6 \pm 3$~ms, $\dims = 200$ takes $829$~ms $\pm 191$~ms.
...
\MT{Talk about which quantities from the main body were calculated via QMC. Make a comparison about how long it would have taken with direct integration.}
